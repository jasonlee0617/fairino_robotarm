...
class MoveIt2:
    ...
    def __init__(
        ...
        self.__retime_service_name = str(retime_service_name)
    ...
    def _retime_trajectory_if_needed(
        self,
        ...
        """
        For ROS2 Humble: cartesian trajectories from compute_cartesian_path often lack proper timing.
        This function optionally calls an external retime service (TOTG/IPP) so that
        max_velocity/max_acceleration scaling factors actually affect execution speed.
        """
        if traj is None:
            return None
        if not cartesian:
        ...
        if not self.__retime_client.wait_for_service(timeout_sec=self.__retime_wait_timeout_sec):
            self._node.get_logger().warn(
                ...
                "executing raw cartesian trajectory (scaling may NOT take effect on Humble)."
            )
            return traj
        ...
        # scaling factors are stored here (your property setters write into these fields)
        ...
        # wait synchronously (consistent with existing plan() style)
        ...
        res = future.result()
        if res is None:
            self._node.get_logger().warn("Retime service returned None; executing raw trajectory.")
            return traj

        # support both 'success' style and fallback
        if hasattr(res, "success") and not res.success:
            self._node.get_logger().warn(
                f"Retime failed: {getattr(res, 'message', 'unknown')}. Executing raw trajectory."
            )
            return traj

        # Optional debug (uncomment if you want)
        # self._node.get_logger().info(
        #     f"Cartesian retimed: vel={req.velocity_scaling:.3f}, acc={req.acceleration_scaling:.3f}, "
        #     f"points={len(res.retimed.points)}"
        # )
        ...
        if cartesian:
            if MoveItErrorCodes.SUCCESS == res.error_code.val:
                if res.fraction >= cartesian_fraction_threshold:
                    ...
                    traj = self._retime_trajectory_if_needed(traj, cartesian=True)
                    return traj
    ...
    def __init_retime_client(self):
        ...
        This requires an external retime server node that provides:
          - service name: self.__retime_service_name (default: /retime_trajectory)
          - service type: trajectory_retime_server/srv/RetimeTrajectory
        """
        if RetimeTrajectory is None:
            self._node.get_logger().warn(
                "RetimeTrajectory service type not available. "
                "Install/compile 'trajectory_retime_server' to enable cartesian retiming."
            )
            return

        self.__retime_client = self._node.create_client(srv_type=RetimeTrajectory,srv_name=self.__retime_service_name,callback_group=self._callback_group,)
